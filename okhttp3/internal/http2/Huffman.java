...
class Huffman {
  ...
  byte[] decode(byte[] paramArrayOfbyte) {
    ...
    while (true) {
      ...
      if (j < paramArrayOfbyte.length) {
        ...
        for (i += 8; i >= 8; i -= 8) {
          node1 = node1.children[k >>> i - 8 & 0xFF];
    ...
    while (m > 0) {
      Node node = node2.children[k << 8 - m & 0xFF];
      if (node.children != null || node.terminalBits > m)
        break; 
  ...
  void encode(f paramf, d paramd) {
    ...
    while (j < paramf.s()) {
      ...
      while (i >= 8) {
        i -= 8;
        paramd.writeByte((int)(l >> i));
      } 
      j++;
    } 
    if (i > 0)
      paramd.writeByte((int)((255 >>> i) | l << 8 - i)); 
  }
  
  int encodedLength(f paramf) {
    long l = 0L;
    for (int i = 0; i < paramf.s(); i++) {
      byte b = paramf.f(i);
      l += CODE_LENGTHS[b & 0xFF];
    } 
    return (int)(l + 7L >> 3L);
  }
...
/* Location:              /home/yc/Downloads/LeaveHomeSafe.jar!/okhttp3/internal/http2/Huffman.class
